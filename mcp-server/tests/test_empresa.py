...
from nfe_brasil._core.errors import FiscalNotFoundError
from nfe_brasil.cnpj.schemas import CNPJResponse
from nfe_brasil.empresa.client import EmpresaClient
from nfe_brasil.simples.schemas import SimplesStatus
...
async def test_get_empresa_success(client):
    with (
        patch("nfe_brasil.cnpj.client.CNPJClient.consultar") as mock_cnpj,
        patch("nfe_brasil.simples.client.SimplesClient.get_simples_status") as mock_simples,
    ):
...
async def test_get_empresa_cnpj_not_found(client):
    with (
        patch("nfe_brasil.cnpj.client.CNPJClient.consultar") as mock_cnpj,
        patch("nfe_brasil.simples.client.SimplesClient.get_simples_status"),
    ):
